...
export const parseTleFile = (fileContent, stationOptions) => {
    ...
    // remove all array elements except the element which contains the string "ISS (ZARYA)" and the next two elements
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes("ISS (ZARYA)")) {
            var lines = lines.splice(0, i);
            var lines = lines.splice(0, 3);
            lines[0] = "ISS - ORBIT";
            console.log(lines);
        }
    }
